
ALTER VIEW CARTEIRA_CLIENTE AS
SELECT ZAO_CODIGO AS CODIGO,ZAO_FILIAL AS EMPRESA,ZAO_VEND AS CODIGO_VENDEDOR,ZAO_CODCLI AS CODIGO_CLIENTE,ZAO_LOJCLI AS LOJA_CLIENTE,
CASE A1_SITUA1
WHEN '1' THEN 'Ativo'
WHEN '2' THEN 'Bloqueado media de vendas'
WHEN '3' THEN 'Bloqueado 90 dias sem compras'
WHEN '4' THEN 'Bloqueado 150 dias sem compras'
WHEN '5' THEN 'Pre Inativo 1 60 dias sem compras'
WHEN '6' THEN 'Pre Inativo 2 70 dias sem compras'
WHEN '7' THEN 'Pre Inativo 3 80 dias sem compras'
WHEN '8' THEN '365 dias sem compras'
WHEN '9' THEN '730 dias sem compras'
WHEN '0' THEN 'Bloqueado sem compras'
END
AS SITUACAO,
RTRIM(ZAO_NOME) AS NOME_CLIENTE,CASE WHEN ZAO_UAGE <> '' THEN CAST(ZAO_UAGE AS DATE) ELSE NULL END AS ULTIMO_AGENDAMENTO ,ZAO_ULTVEN AS ULTIMO_VENDEDOR,
ZAO_ULTFIL AS ULTIMA_FILIAL,RTRIM(ZAO_OBS) AS OBSERVACAO,ZAO.R_E_C_N_O_ AS REC,A1_DDD+A1_TEL AS TELEFONE
FROM ZAO010 ZAO (NOLOCK) INNER JOIN SA1010  A1 ON ZAO_CODCLI = A1_COD AND ZAO_LOJCLI = A1_LOJA
WHERE ZAO.D_E_L_E_T_ = '' AND A1.D_E_L_E_T_ = ''